...
*/
(function(w){
	var E=w(window),u,f,F=-1,n,x,D,v,y,L,r
					...
					J=new Image(),
					H,a,g,p,I,d,G,c,A,K;
					w(function(){w("body").append(
						w([H=w('<div id="lbOverlay" />').click(C)[0],
						a=w('<div id="lbCenter" />')[0],
						G=w('<div id="lbBottomContainer" />')[0]]).css("display","none"));
						g=w('<div id="lbImage" />').appendTo(a).append(
							p=w('<div style="position: relative;" />'
						).append([
							d=w('<a id="lbPrevLink" href="" />').click(e)[0],
							I=w('<a id="lbNextLink" href="" />').click(B)[0]
						])[0])[0];
						...
						w.slimbox=function(O,N,M){
							u=w.extend({loop:false,
								...
								nextKeys:[39,78]},M);
							if(typeof O=="string"){
								O=[[O,N]];N=0}y=E.scrollTop()+(E.height()/2);
								L=u.initialWidth;
								r=u.initialHeight;
								w(a).css({
							...
							w.fn.slimbox=function(M,P,O){
								...
								function z(){
									...
									if(v){
										...
										function j(M){
											if(M){
												w("object").add(m?"select":"embed").each(
													function(O,P){
										...
										function i(){
											...
											w(a).queue(function(){
												w(G).css({
													width:L,top:M+r,marginLeft:-L/2,visibility:"hidden",display:""
												});
										...
										w(c).css("marginTop",-c.offsetHeight).animate({marginTop:0},
